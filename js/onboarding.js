  ...
  const TOURS = {
    ...
    study: [
      { target: '#chapterRail', title: '章节',
        body: '上面那一行是现在这一章，点它就会展开全部章节。每张卡底下写著你掌握到哪：还没开始写题数，做了一半有进度条，全部掌握会打勾。',
        before: () => window.setChapterPanel && window.setChapterPanel(true),
        after: () => window.setChapterPanel && window.setChapterPanel(false) },
