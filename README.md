...
| 功能 | 说明 |
|---|---|
| 选择题练习 | 按章练习；选对才锁定并展开考点解析，选错可以重选 |
...
| 同步码 | 不用帐号，用一组同步码（可扫 QR）在手机与电脑之间同步进度与笔记 |
...
AI 产出的每一题都要经过你按「采纳」才会上线。已上线的题在 `/dev`「题目修改」可以直接改，不用碰 JSON。
...
api/                                            Vercel 接口：登录、审题、改题、AI 批改、同步
dev/                                            开发者工作台
...
| Vercel | `GEMINI_API_KEY` | 做答题的 AI 批改 |
| Vercel | `UPSTASH_REDIS_REST_URL` `UPSTASH_REDIS_REST_TOKEN` | 跨装置同步、AI 批改的次数限制 |
