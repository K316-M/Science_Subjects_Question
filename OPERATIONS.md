...
**👋 首次引导** —— 这台装置第一次打开网站时，会自动跑一次四页的引导（选科 → 做题 → 复习 → 同步），按「跳过」或走完就不再出现。
...
（这一段只适用於还在用 background.svg 的化学、物理科。用 scene.json 的科目，大屏也会动：题卡等大块卡片自动改成不透明的纸面，见下一节。）

### 会动的分层背景（scene.json）

生物科现在是一张水彩背景拆成的 17 个图层：大的（纸、山丘、叶丛、DNA）不动，小的（飘叶、细胞、光点）慢慢摇、飘、晃。
...
1. 用 Gemini 或 ChatGPT 生成一张参考图（提示词范本在下面），跟它对话修到满意
2. 把图交给我——拆图层、设定动作、手机版摆位、量帧数、跑一键检查都是我来做

你也可以自己跑拆图管线（`scripts/scene/build_scene.py`，用法写在档案开头），但决定「哪些元素组成一个图层、怎么动」需要看图判断，交给我比较快。

参考图提示词范本（把「主题」那行换成化学或物理）：

```
用途：学习网站的背景，内容会盖在画面中央。
画风：柔和水彩加细颗粒，光线平均、没有强烈的方向光。
色调：只用 #ECFDF5、#A7F3D0、#047857、#F6EFDD，低饱和。
构图：16:9。画面中央 60% 保持干净、低对比（文字会叠在上面）；
      元素只放在左右两侧与底部；底部是连绵的山丘。
      左右两侧放一些「独立、不相连」的小元素（小叶片、小圆点、小符号），它们之後会被做成会动的图层。
主题：化学——六角形分子结构、烧瓶的抽象轮廓、气泡。不要写实的实验器材。
禁止：任何文字、字母、数字、浮水印、边框。
```
