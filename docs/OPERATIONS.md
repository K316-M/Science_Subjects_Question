...
两条流水线的产物都停在待审区 `papers/pending_approval.json`。到**工作台 → AI 录题待审**处理：
...
- **要处理的题** —— 被自动检查标出问题的（答案有疑、和原档不一致、夹英文、题号没去掉、没判断出章节……），
  每题直接展开成编辑器：改题干、选项、答案、解析，**上传或换配图**，改好按「采纳（连同修改）」一次写进题库
- **没被标出问题的题** —— 收成一张清单，题干与正解（绿色）都列出来，扫过一遍按「采纳勾选的 N 题」；
  有疑问的取消勾选，或按「编辑」展开来改
...
题库、待审区、配图在**同一个 commit** 里写进去：一次采纳（不管几题）只部署一次，也不会出现写一半的状态。

### 4. 修改已上线的题

不用到 GitHub 找 JSON 档。到**工作台 → 题目修改**，打几个字搜寻题目，或选一科、再选章节把整章列出来，
按「编辑」就能改文字、答案、解析、换配图，按「存档并上线」约一分钟後学生就看得到。**题库巡检**列出的问题题也有「编辑」按钮，可以直接修。

**不要的题按「下架」**：学生马上看不到它（练习、错题本、今日复习、测验、模拟统考、题目档、AI 批改／讲解都不会再出现），
题目本身留在 JSON 里、加上 `"hidden": true`。勾「只看已下架的题」可以找回来按「恢复」。

> **为什么不真的删掉**：学生的进度、错题、笔记都是按「这一章第几题」记在各自装置上的。从 JSON 删掉第 3 题，
> 原本的第 4、5、6 题就变成第 3、4、5 题，每个学生的纪录都会对到别题上，而且没办法从服务器修回来。
...
> 在 /dev 改过这题的题干、选项、答案或解析，旧讲解自动作废；内容没改但讲错了，到「题目修改」→ 编辑 →「清除 AI 讲解」。
...
学生怎么用：在一台装置按工具列的「同步 → 产生我的同步码」，然後三选一把另一台装置接上：

- **扫 QR 码**（电脑 → 手机最快）：面板上会画出一个 QR 码，用手机相机扫就会打开网站并问「要接上吗？」
- **复制同步链结**：传给自己（例如 WhatsApp 给自己），在另一台装置打开即可
- **手动输入**那 20 个字的同步码

两边的进度会**合并**而不是覆盖——答对过的题目不会因为另一台没做过而被洗掉。半年没同步的资料会自动过期。
